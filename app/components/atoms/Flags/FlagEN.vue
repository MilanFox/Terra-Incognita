<template>
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="100%" height="100%" fill="none">
    <circle cx="256" cy="256" r="256" fill="var(--flag-palette-blue, #0052b4)" />

    <path fill="#EEE" d="M459.4 100.6 304 256l155.4 155.4q-20.9 27.1-48 48L256 304 100.6 459.4q-27.2-20.9-48-48L208 256 52.6 100.6q20.8-27.2 48-48L256 208 411.4 52.6q27.1 20.8 48 48" />

    <path fill="#EEE" d="M288 2q16.4 2 32 6v184h184q4 15.6 6 32l-30 32 30 32a255 255 0 0 1-6 32H320v184a255 255 0 0 1-32 6l-32-30-32 30a255 255 0 0 1-32-6V320H8a255 255 0 0 1-6-32l30-32-30-32q2-16.4 6-32h184V8q15.6-4 32-6l32 30z" />

    <path fill="var(--flag-palette-bright-red, #d80027)" d="M288 2q-15.5-2-31.5-2h-1q-16 0-31.5 2v222H2a259 259 0 0 0 0 64h222v222a259 259 0 0 0 64 0V288h222a258 258 0 0 0 0-64H288z" />

    <path fill="var(--flag-palette-bright-red, #d80027)" fill-rule="evenodd" d="M420.3 59.7Q429 67 437 75L320 192v-32zm-360.6 32Q67 83 75 75l117 117h-32zM437 437q8.1-8 15.3-16.7L352 320h-32zM91.7 452.3 192 352v-32L75 437q8 8.1 16.7 15.3" clip-rule="evenodd" />
  </svg>
</template>
